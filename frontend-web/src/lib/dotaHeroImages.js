// Official Dota 2 hero portraits served from Steam's CDN.
// Map: OpenDota hero_id -> internal npc short name (the part after npc_dota_hero_).
// Images load in the browser at runtime, so no large binaries live in the repo.
...
const HERO_SLUG = {
  ...
  109: 'terrorblade', 110: 'phoenix', 111: 'oracle', 113: 'arc_warden',
  114: 'monkey_king', 119: 'dark_willow', 120: 'pangolier', 121: 'grimstroke',
  123: 'hoodwink', 126: 'void_spirit', 128: 'snapfire', 129: 'mars',
...
// Full-body vertical portrait (great for the 3D card).
export function heroPortrait(id) {
  const slug = HERO_SLUG[id]
  return slug ? `${CDN}/${slug}.png` : ''
}

// Small horizontal icon.
export function heroIcon(id) {
  const slug = HERO_SLUG[id]
  return slug ? `${CDN}/icons/${slug}.png` : ''
}

export function heroSlug(id) {
  return HERO_SLUG[id] || ''
}
