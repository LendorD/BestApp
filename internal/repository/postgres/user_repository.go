...
func (r *UserRepository) CreateUser(ctx context.Context, input domain.RegisterUserInput, passwordHash string) (*domain.User, error) {
	row := r.pool.QueryRow(ctx, `
		INSERT INTO tbl_users (
			email, username, password_hash, display_name, updated_at
		)
		VALUES ($1, $2, $3, $4, now())
...
func (r *UserRepository) GetUserByIdentity(ctx context.Context, identity string) (*domain.User, string, error) {
	row := r.pool.QueryRow(ctx, `
		SELECT id, email, username, display_name, avatar_url, bio, favorite_game,
...
func (r *UserRepository) UpdateProfile(ctx context.Context, id int64, input domain.UpdateUserProfileInput) (*domain.User, error) {
	row := r.pool.QueryRow(ctx, `
		UPDATE tbl_users
		SET display_name = $2,
		    avatar_url = $3,
...
func scanUserWithPassword(row scanner) (*domain.User, string, error) {
	return scanUserFields(row, true)
}

func scanUserFields(row scanner, withPassword bool) (*domain.User, string, error) {
